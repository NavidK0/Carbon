...
public abstract class PerfDataBufferImpl {
	...
	/**
	 * Get the Local Java Virtual Machine Identifier, or <em>lvmid</em> for the target JVM associated with this instrumentation buffer.
	 *
	...
	public byte[] getBytes() {
		...
		synchronized (this) {
			...
			try {
				...
				/*
				 * just ignore this here and let the reciever of the byte[] detect and handle the problem.
				 */
			}
			bb = buffer.duplicate();
		}
	 ...
	 *
	 * @return ByteBuffer - a ByteBuffer object that refers to the instrumentation data.
	 */
	...
	private void buildAliasMap() {
		...
		if (filename != null) {
			...
			aliasURL = getClass().getResource("/sun/jvmstat/perfdata/resources/aliasmap");
		}
	 ...
	 *
	 * @return MonitorStatus - the status of available Monitors for the target Java Virtual Machine.
	 * @throws MonitorException
