 ...
 * <p>
 * Provides a factory for creating a File object to the backing store file for instrumentation shared memory region for a JVM identified by its Local Java Virtual Machine Identifier, or <em>lvmid</em>.
 *
...
public class PerfDataFile {
	 ...
	 * <p>
	 * This method looks for the most up to date backing store file for the given <tt>lvmid</tt>. It will search all the user specific directories in the temporary directory for the host operating system, which may be influenced by platform specific environment variables.
	 *
	 * @param lvmid
	 *            the local Java Virtual Machine Identifier for the target
	 * @return File - a File object to the backing store file for the named shared memory region of the target JVM.
	 * @see java.io.File
	...
	public static File getFile(int lvmid) {
		if (lvmid == 0) {
			/*
			 * lvmid == 0 is used to indicate the current Java Virtual Machine. If the SDK provided an API to get a unique Java Virtual Machine identifier, then a filename could be constructed with that identifier. In absence of such an api, return null.
			 */
			return null;
		}

		/*
		 * iterate over all files in all directories in tmpDirName that match the file name patterns.
		 */
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (f.exists() && f.isDirectory()) {
				/*
				 * found a directory matching the name patterns. This is a 1.4.2 hsperfdata_<user> directory. Check for file named <lvmid> in that directory
				 */
				...
				/*
				 * found a file matching the name patterns. This is a 1.4.1 hsperfdata_<lvmid> file.
				 */
				candidate = f;
	 ...
	 *
	 * @param file
	 *            A File object representing the name of a shared memory region for a target JVM
	 * @return int - the local Java Virtual Machine Identifier for the target associated with the file
	...
	/*
	 * this static initializer would not be necessary if the Solaris java.io.tmpdir property were set to /tmp by default
	 */
	static {
		/*
		 * Why is java.io.tmpdir on Solaris set to "/var/tmp/" when the HotSpot JVM os:get_temp_path() method returns "/tmp/"
		 * 
		 * Why do Solaris and Windows return a string with a trailing file separator character where as Linix does not? (this change seems to have occurred sometime during hopper beta)
		 */
		String tmpdir = System.getProperty("java.io.tmpdir");

		if (tmpdir.compareTo("/var/tmp/") == 0) {
			/*
			 * shared memory files are created in /tmp. Interestingly, java.io.tmpdir is set to "/var/tmp/" on Solaris and Linux, but os::get_temp_directory() is set to "/tmp/" on these platforms. the java.io.logging packages also makes reference to java.io.tmpdir.
			 */
			tmpdir = "/tmp/";
		}

		/*
		 * Assure that the string returned has a trailing File.separator character. This check was added because the Linux implementation changed such that the java.io.tmpdir string no longer terminates with a File.separator character.
		 */
