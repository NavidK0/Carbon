...
/**
 * Concrete implementation of the AbstractMonitoredVm class for the <em>rmi:</em> protocol for the HotSpot PerfData monitoring implementation.
 * <p>
...
public class RemoteMonitoredVm extends AbstractMonitoredVm {
	 ...
	 *
	 * @param rvm
	 *            the proxy to the remote MonitoredVm instance.
	 * @param vmid
	 *            the vm identifier specifying the remot target JVM
	 * @param timer
	 *            the timer used to run polling tasks
	 * @param interval
	 ...
	 *             Thrown on any communications errors with the remote system.
	 */
	...
	public void setInterval(int newInterval) {
		synchronized (listeners) {
			...
			if (samplerTask != null) {
				...
				samplerTask = new SamplerTask();
				CountedTimerTaskUtils.reschedule(timer, oldSamplerTask, samplerTask, oldInterval, newInterval);
			}
			if (notifierTask != null) {
				notifierTask.cancel();
				NotifierTask oldNotifierTask = notifierTask;
				notifierTask = new NotifierTask();
				CountedTimerTaskUtils.reschedule(timer, oldNotifierTask, notifierTask, oldInterval, newInterval);
			}
		}
	}
	...
	/*
	 * Timer Tasks. There are two separate timer tasks here. The SamplerTask is active whenever we are attached to the remote JVM with a periodic sampling interval > 0. The NotifierTask is only active if a VmListener has registered with this RemoteMonitoredVm instance. Also, in the future we may want to run these tasks at different intervals. Currently, they run at the same interval and some significant work may need to be done to complete the separation of these two intervals.
	 */
