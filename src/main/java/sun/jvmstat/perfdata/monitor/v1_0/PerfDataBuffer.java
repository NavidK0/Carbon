...
public class PerfDataBuffer extends PerfDataBufferImpl {
		...
		if ((used > lastUsed) || (lastModificationTime > modificationTime)) {
			...
			while (monitor != null) {
				...
				if (!map.containsKey(name)) {
					...
					/*
					 * insertedMonitors is null when called from pollFor() via buildMonitorMap(). Since we update insertedMonitors at the end of buildMonitorMap(), it's ok to skip the add here.
					 */
	 ...
	 *
	 * gross synchronization works by polling for the hotspot.rt.hrt.ticks counter, which is the last counter created by the StatSampler initialization code. The counter is updated when the watcher thread starts scheduling tasks, which is the last thing done in vm initialization.
	 */
	protected void synchWithTarget(Map<String, Monitor> map) throws MonitorException {
		...
		/*
		 * loop waiting for the ticks counter to be non zero. This is an indication that the jvm is initialized.
		 */
	...
	/**
	 * method to repair the 1.4.2 parallel scavenge counters that are incorrectly initialized by the JVM when UseAdaptiveSizePolicy is set. This bug couldn't be fixed for 1.4.2 FCS due to putback restrictions.
	 */
	private void kludgeMantis(Map<String, Monitor> map, StringMonitor args) {
		/*
		 * the HotSpot 1.4.2 JVM with the +UseParallelGC option along with its default +UseAdaptiveSizePolicy option has a bug with the initialization of the sizes of the eden and survivor spaces. See bugid 4890736.
		 * 
		 * note - use explicit 1.4.2 counter names here - don't update to latest counter names or attempt to find aliases.
		 */
		...
		if (collector.stringValue().compareTo("PSScavenge") == 0) {
			...
			/*
			 * HotSpot processes the -XX:Flags/.hotspotrc arguments prior to processing the command line arguments. This allows the command line arguments to override any defaults set in .hotspotrc
			 */
			...
			/*
			 * ignore the -XX: prefix as it only applies to the arguments passed from the command line (i.e. the invocation api). arguments passed through .hotspotrc omit the -XX: prefix.
			 */
			...
			if (ahi != -1) {
				/*
				 * +AggressiveHeap was set, check if -UseAdaptiveSizePolicy is set after +AggressiveHeap.
				 */
				//
				if ((aspi != -1) && (aspi > ahi)) {
				...
				/*
				 * +AggressiveHeap not set, must be +UseParallelGC. The relative position of -UseAdaptiveSizePolicy is not important in this case, as it will override the UseParallelGC default (+UseAdaptiveSizePolicy) if it appears anywhere in the JVM arguments.
				 */
				if (aspi != -1) {
					adaptiveSizePolicy = false;
				}
