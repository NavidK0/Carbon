...
public class PerfDataBuffer extends PerfDataBufferImpl {
	 ...
	 * <p>
	 * This class is dynamically loaded by {@link AbstractPerfDataBuffer#createPerfDataBuffer}, and this constructor is called to instantiate the instance.
	 *
	 * @param buffer
	...
	protected void synchWithTarget() throws MonitorException {
		/*
		 * synch must happen with syncWaitMs from now. Default is 5 seconds, which is reasonabally generous and should provide for extreme situations like startup delays due to allocation of large ISM heaps.
		 */
	...
	protected Monitor getNextMonitorEntry() throws MonitorException {
		...
		if ((nextEntry % 4) != 0) {
			throw new MonitorStructureException("Misaligned entry index: " + Integer.toHexString(nextEntry));
		}

		// protect againt a corrupted shard memory region.
		if ((nextEntry < 0) || (nextEntry > buffer.limit())) {
			throw new MonitorStructureException("Entry index out of bounds: " + Integer.toHexString(nextEntry) + ", limit = " + Integer.toHexString(buffer.limit()));
		}
		...
		if ((entryStart + entryLength) > buffer.limit()) {
			throw new MonitorStructureException("Entry extends beyond end of buffer: " + " entryStart = 0x" + Integer.toHexString(entryStart) + " entryLength = 0x" + Integer.toHexString(entryLength) + " buffer limit = 0x" + Integer.toHexString(buffer.limit()));
		}
		...
		dump_entry_fixed(entryStart, nameOffset, vectorLength, typeCodeByte, flags, unitsByte, varByte, dataOffset);
		...
		try {
			...
			throw new MonitorStructureException("Illegal type code encountered:" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + ", type_code = " + Integer.toHexString(typeCodeByte));
		}

		// verify that the name_offset is contained within the entry bounds
		if (nameOffset > entryLength) {
			throw new MonitorStructureException("Field extends beyond entry bounds" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + ", name_offset = 0x" + Integer.toHexString(nameOffset));
		}

		// verify that the data_offset is contained within the entry bounds
		if (dataOffset > entryLength) {
			throw new MonitorStructureException("Field extends beyond entry bounds:" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + ", data_offset = 0x" + Integer.toHexString(dataOffset));
		}

		// validate the variability and units fields
		if (variability == Variability.INVALID) {
			throw new MonitorDataException("Invalid variability attribute:" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + ", variability = 0x" + Integer.toHexString(varByte));
		}

		if (units == Units.INVALID) {
			throw new MonitorDataException("Invalid units attribute: entry_offset = 0x" + Integer.toHexString(nextEntry) + ", units = 0x" + Integer.toHexString(unitsByte));
		}

		// the entry looks good - parse the variable length components

		/*
		 * The name starts at nameOffset and continues up to the first null byte. however, we don't know the length, but we can approximate it without searching for the null by using the offset for the data field, which follows the name field.
		 */
		...
		/*
		 * compute the size of the data item - this includes pad characters used to align the next entry.
		 */
		...
		if (vectorLength == 0) {
			...
			if (typeCode == TypeCode.LONG) {
				...
				monitor = new PerfLongMonitor(name, units, variability, supported, lb);
			} else {
				/*
				 * unexpected type code - coding error or uncoordinated JVM change
				 */
				throw new MonitorTypeException("Unexpected type code encountered:" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + ", name = " + name + ", type_code = " + typeCode + " (0x" + Integer.toHexString(typeCodeByte) + ")");
			}
		} else {
			...
			if (typeCode == TypeCode.BYTE) {
				if (units != Units.STRING) {
					...
					throw new MonitorTypeException("Unexpected vector type encounterd:" + " entry_offset = " + Integer.toHexString(nextEntry) + ", name = " + name + ", type_code = " + typeCode + " (0x" + Integer.toHexString(typeCodeByte) + ")" + ", units = " + units + " (0x" + Integer.toHexString(unitsByte) + ")");
				}
				...
				if (variability == Variability.CONSTANT) {
					...
					throw new MonitorDataException("Unexpected variability attribute:" + " entry_offset = 0x" + Integer.toHexString(nextEntry) + " name = " + name + ", variability = " + variability + " (0x" + Integer.toHexString(varByte) + ")");
				} else {
	...
	private void dump_entry_fixed(int entry_start, int nameOffset, int vectorLength, byte typeCodeByte, byte flags, byte unitsByte, byte varByte, int dataOffset) {
		if (DEBUG) {
			System.err.println("Entry at offset: 0x" + Integer.toHexString(entry_start));
			System.err.println("\tname_offset = 0x" + Integer.toHexString(nameOffset));
			System.err.println("\tvector_length = 0x" + Integer.toHexString(vectorLength));
			System.err.println("\tdata_type = 0x" + Integer.toHexString(typeCodeByte));
			System.err.println("\tflags = 0x" + Integer.toHexString(flags));
			System.err.println("\tdata_units = 0x" + Integer.toHexString(unitsByte));
			System.err.println("\tdata_variability = 0x" + Integer.toHexString(varByte));
			System.err.println("\tdata_offset = 0x" + Integer.toHexString(dataOffset));
		}
	}
	...
	private void dump_entry_variable(String name, ByteBuffer bb, int size) {
		if (DEBUG) {
			...
			while (data.hasRemaining()) {
				...
				System.err.print(String.valueOf(toHex[high]) + String.valueOf(toHex[low]));
