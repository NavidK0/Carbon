...
/**
 * An abstraction for a host that contains instrumented Java Virtual Machines. The class provides abstract factory methods for creating concrete instances of this class and factory methods for creating {@link MonitoredVm} instances. Concrete implementations of this class provide methods for managing the communications protocols and provide for event notification.
 *
...
public abstract class MonitoredHost {
	private static Map<HostIdentifier, MonitoredHost> monitoredHosts = new HashMap<HostIdentifier, MonitoredHost>();

	/*
	 * The monitoring implementation override mechanism. The value of this property is used as the class name for the concrete MonitoredHost subclass that implements the monitoring APIs. Setting this property will cause the remaining override mechanisms to be ignored. When this mechanism is used, the HostIdentifier scheme name, which indicates the communications protocol, is not used to locate a the protocol specific package. However, the HostIdentifier is still passed to the corresponding
	 * single arg constructor. This property is not expected to be set in normal circumstances.
	 */
	private static final String IMPL_OVERRIDE_PROP_NAME = "sun.jvmstat.monitor.MonitoredHost";

	/*
	 * The monitoring package name override mechanism. The value the this property is used as base package name for the monitoring implementation package. This property is not expected to be set under normal circumstances.
	 */
	private static final String IMPL_PKG_PROP_NAME = "sun.jvmstat.monitor.package";
	private static final String IMPL_PACKAGE = System.getProperty(IMPL_PKG_PROP_NAME, "sun.jvmstat.perfdata");

	/*
	 * The default optimized local protocol override mechanism. The value of this property is used to construct the default package name for the default optimized local protocol as follows: <IMPL_PACKAGE>.monitor.<LOCAL_PROTOCOL> This property is not expected to be set under normal circumstances.
	 */
	private static final String LOCAL_PROTOCOL_PROP_NAME = "sun.jvmstat.monitor.local";
	private static final String LOCAL_PROTOCOL = System.getProperty(LOCAL_PROTOCOL_PROP_NAME, "local");

	/*
	 * The default remote protocol override mechanism. The value of this property is used to construct the default package name for the default remote protocol protocol as follows: <IMPL_PACKAGE>.monitor.protocol.<REMOTE_PROTOCOL> This property is not expected to be set under normal circumstances.
	 */
	private static final String REMOTE_PROTOCOL_PROP_NAME = "sun.jvmstat.monitor.remote";
	private static final String REMOTE_PROTOCOL = System.getProperty(REMOTE_PROTOCOL_PROP_NAME, "rmi");

	/*
	 * The default class name of the MonitoredHost implementation subclass. There is no override mechanism for this variable, other than the IMPL_OVERRIDE_PROP_NAME override, which is larger in scope. A concrete instance of this class is expected to be found in: <IMPL_PACKAGE>.monitor.protocol.<protocol>.<MONITORED_HOST_CLASS>
	 */
	...
	/**
	 * Factory method to construct MonitoredHost instances to manage connections to the host indicated by <tt>hostIdString</tt>
	 *
	 * @param hostIdString
	 *            a String representation of a {@link HostIdentifier}
	 * @return MonitoredHost - the MonitoredHost instance for communicating with the indicated host using the protocol specified in hostIdString.
	...
	/**
	 * Factory method to construct a MonitoredHost instance to manage the connection to the Java Virtual Machine indicated by <tt>vmid</tt>.
	 *
	 * This method provide a convenient short cut for attaching to a specific instrumented Java Virtual Machine. The information in the VmIdentifier is used to construct a corresponding HostIdentifier, which in turn is used to create the MonitoredHost instance.
	 *
	 * @param vmid
	 *            The identifier for the target Java Virtual Machine.
	 * @return MonitoredHost - The MonitoredHost object needed to attach to the target Java Virtual Machine.
	 *
	 * @throws MonitorException
	 *             Thrown if monitoring errors occur.
	 */
	...
	/**
	 * Factory method to construct a MonitoredHost instance to manage the connection to the host indicated by <tt>hostId</tt>.
	 *
	 * @param hostId
	 *            the identifier for the target host.
	 * @return MonitoredHost - The MonitoredHost object needed to attach to the target host.
	 *
	 * @throws MonitorException
	 *             Thrown if monitoring errors occur.
	 */
	public static MonitoredHost getMonitoredHost(HostIdentifier hostId) throws MonitorException {
		/*
		 * determine the class name to load. If the system property is set, use the indicated class. otherwise, use the default class.
		 */
		...
		if (classname == null) {
			...
			classname = IMPL_PACKAGE + ".monitor.protocol." + hostId.getScheme() + "." + MONITORED_HOST_CLASS;
		}
	 ...
	 *
	 * @return Exception - the last exception occurred while polling this MonitoredHost, or <tt>null</tt> if no exception has occurred or the exception has been cleared,
	 */
	 ...
	 *
	 * @return boolean - true if the MonitoredHost instance has experienced an error, or false if it hasn't or if any past error has been cleared.
	 */
	 ...
	 * @return MonitoredVm - the MonitoredVm instance for the target Java Virtual Machine.
	 * @throws MonitorException
	 *             Thrown if monitoring errors occur.
	 */
	...
	/**
	 * Return the current set of active Java Virtual Machines for this MonitoredHost. The returned Set contains {@link Integer} instances holding the local virtual machine identifier, or <em>lvmid</em> for each instrumented Java Virtual Machine currently available.
	 *
	 * @return Set - the current set of active Java Virtual Machines associated with this MonitoredHost, or the empty set of none.
	 * @throws MonitorException
	 *             Thrown if monitoring errors occur.
	 */
