 ...
 * </pre>
 * 
 * The only required component of this string is the Local Virtual Machine Identifier, or <tt>lvmid</tt>, which uniquely identifies the target Java Virtual Machine on a host. The optional components of the VmIdentifier include:
 * <ul>
 * <li>
 * <p>
 * <tt>protocol</tt> - The communications protocol. A VmIdentifier omitting the protocol must be resolved against a HostIdentifier using {@link HostIdentifier#resolve}.
 * </p>
 * </li>
 * <li>
 * <p>
 * <tt>hostname</tt> - A hostname or IP address indicating the target host. A VmIdentifier omitting the protocol must be resolved against a HostIdentifier using {@link HostIdentifier#resolve}.
 * </p>
 * </li>
 * <li>
 * <p>
 * <tt>port</tt> - The port for the communications protocol. Treatment of the <tt>port</tt> parameter is implementation (protocol) specific. A VmIdentifier omitting the protocol should be resolved against a HostIdentifier using {@link HostIdentifier#resolve}.
 * </p>
 * </li>
 * <li>
 * <p>
 * <tt>servername</tt> - The treatment of the Path, Query, and Fragment components of the VmIdentifier are implementation (protocol) dependent. A VmIdentifier omitting the protocol should be resolved against a HostIdentifier using {@link HostIdentifier#resolve}.
 * </p>
 * </li>
 * </ul>
 * <p>
 ...
 * </p>
 * <p>
 * With the exception of <em>file:</em> based VmIdentifier strings, all VmIdentifier strings must include a <tt>lvmid</tt>. Attempting to construct a non-file based VmIdentifier that doesn't include a <tt>lvmid</tt> component will result in a <tt>MonitorException</tt>.
 * </p>
 * <p>
 ...
 * <li>
 * <p>
 * <em>1234</em> - Specifies the Java Virtual Machine identified by lvmid <em>1234</em> on an unnamed host. This string is transformed into the absolute form <em>//1234</em>, which must be resolved against a HostIdentifier.
 * </p>
 * </li>
 * <li>
 * <p>
 * <em>1234@hostname</em> - Specifies the Java Virtual Machine identified by lvmid <em>1234</em> on host <em>hostname</em> with an unnamed protocol. This string is transformed into the absolute form <em>//1234@hostname</em>, which must be resolved against a HostIdentifier.
 * </p>
 * </li>
 * <li>
 * <p>
 * <em>1234@hostname:2099</em> - Specifies the Java Virtual Machine identified by lvmid <em>1234</em> on host <em>hostname</em> with an unnamed protocol, but with port <em>2099</em>. This string is transformed into the absolute form <em>//1234@hostname:2099</em>, which must be resolved against a HostIdentifier.
 * </p>
 * </li>
 * </ul>
 * <li>
 ...
 * <li>
 * <p>
 * <em>rmi://1234@hostname:2099/remoteobjectname</em> - Specifies the Java Virtual Machine identified by lvmid <em>1234</em> on host <em>hostname</em> accessed using the <em>rmi:</em> protocol through the rmi remote object named <em>remoteobjectname</em> as registered with the <em>rmiserver</em> on port <em>2099</em> on host <em>hostname</em>.
 * </p>
 * </li>
 * <li>
 * <p>
 * <em>file:/path/file</em> - Identifies a Java Virtual Machine through accessing a special file based protocol to use as the communications mechanism.
 * </p>
 * </li>
 * </ul>
 * </ul>
...
public class VmIdentifier {
	...
	private URI canonicalize(String uriString) throws URISyntaxException {
		...
		if (u.isAbsolute()) {
			if (u.isOpaque()) {
				/*
				 * rmi:1234@hostname/path#fragment converted to rmi://1234@hostname/path#fragment
				 */
				u = new URI(u.getScheme(), "//" + u.getSchemeSpecificPart(), u.getFragment());
			}
		} else {
			/*
			 * make the uri absolute, if possible. A relative URI doesn't specify the scheme part, so it's safe to prepend a "//" and try again.
			 */
			if (!uriString.startsWith("//")) {
				if (u.getFragment() == null) {
					u = new URI("//" + u.getSchemeSpecificPart());
				} else {
					u = new URI("//" + u.getSchemeSpecificPart() + "#" + u.getFragment());
				}
			}
		}
		return u;
	...
	public VmIdentifier(String uriString) throws URISyntaxException {
		...
		try {
			...
			/*
			 * a vmid of the form 1234@hostname:1098 causes an exception, so try again with a leading "//"
			 */
			if (uriString.startsWith("//")) {
				throw e;
			}
	 ...
	 * </p>
	 * <p>
	 * For example, a VmIdentifier that specifies only a <tt>lvmid</tt> will result in a HostIdentifier for <em>localhost</em> utilizing the default local protocol, <em>local:</em>. A VmIdentifier that specifies both a <tt>vmid</tt> and a <tt>hostname</tt> will result in a HostIdentifier for the specified host with the default remote protocol, <em>rmi:</em>, using the protocol defaults for the <tt>port</tt> and <tt>servername</tt> components.
	 * </p>
	 *
	 * @return HostIdentifier - the host identifier for the host containing the Java Virtual Machine represented by this VmIdentifier.
	 * @throws URISyntaxException
	...
	/**
	 * Return the Local Virtual Machine Identifier for this VmIdentifier. The Local Virtual Machine Identifier is also known as the <em>lvmid</em>.
	 *
	 ...
	 *
	 * @return String - the mode string. If no mode is specified, then "r" is returned. otherwise, the specified mode is returned.
	 */
	 ...
	 * @return boolean - returns true if the given object is of type VmIdentifier and its URI field is equal to this object's URI field. Otherwise, return false.
	 *
